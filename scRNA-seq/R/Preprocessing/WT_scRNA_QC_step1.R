...
cr_path <- file.path("/your_work_dir/data/Peritoneal_ChAT/CRcount/Neg/outs/filtered_feature_bc_matrix")

## ! path to loom
loom_path <- file.path("/your_work_dir/data/Peritoneal_ChAT/CRcount/Neg/velocyto/Neg.loom")


# set your work diretory
workdir <- file.path("/your_work_dir/data/scRNAseq-R", sample)
setwd(workdir)
...
if (species == "mm") {
  ...
  load("/your_work_dir/Ref_genome/CellType_ref/Immgen_ref/ImmGen_reference-Heng_2008.RData")
  pred <- SingleR(
    test = sce,
    ref = immgen,
...
if (species == "hs") {
  ...
  seu.annot <- RunAzimuth(
    seu,
    reference = "/your_work_dir/Ref_genome/CellType_ref/Azimuth_ref/Human_PBMC"
  )
...
# Run in server
# nohup Rscript --vanilla /your_work_dir/data/scRNAseq-R/Neg/Neg_scRNA_QC_step2.R &
