...
workdir <- "/your_work_dir/data/Peritoneal_ChAT/R"
setwd(workdir)
...
scenicLoomPath <- file.path(SCENICdir, 'KOnWT_int.scenic.loom')
motifEnrichmentFile <- file.path(SCENICdir, 'KOnWT_int.motifs.csv')
regulonAucFile <- file.path(SCENICdir, 'KOnWT_int.auc.csv')
BinarymatFile <- file.path(SCENICdir, 'KOnWT_int.bin.csv')
regulonAucThresholdsFile <- file.path(SCENICdir, 'KOnWT_int.thresholds.csv')
...
save(regulons_incidMat, regulons, regulonAUC, motifEnrichment, regulonAUC2, regulonBin,AUCmat,
  file = "03_KOnWT_pySCENIC_results.RData"
)
...
# load seurat object
library('Seurat')
load("03_KOnWT_pySCENIC_results.RData")
...
saveRDS(seu_int, file = "03_KO_pySCNIEC2seurat.rds")
